...
|---|---|---|---|
| 用户 | 人类用户 | 登录系统、查看日历、个人记事、申报费用、管理任务、管理纪念日 | 本系统不与外部系统直接交互，无外部接口需求 |

## 3. 术语表
| 术语 | 定义 | 备注 |
|---|---|---|
| 项目成员 | 团队成员，可参与项目任务 | 分为普通成员和项目经理 |
| 项目经理 | 项目负责人，拥有项目计划管理权限 | 可编辑所有任务数据 |
...
*   **BR-005（数据来源）：** 日历数据从个人记事、项目计划、公司任务、纪念日中聚合显示。
...
**参与者：** 项目成员、项目经理  
**主要目标：** 填报项目进展。  
**前置条件：** 项目成员/项目经理已成功登录系统。

##### 主成功场景
1. 项目成员/项目经理进入"填报项目进展"界面。
2. 系统显示完整项目计划，包含：
   - 项目全称、简称
   - 阶段、任务
   - 计划和实际数据
...
*   **4b. 删除用户：**
    1. 管理员选择用户进行删除。
    2. 系统将用户标记为"已离职"，保留历史关联数据。

##### 业务规则
*   **BR-021（用户名规则）：** 用户名必须唯一，使用无业务含义的ID作为主键。
*   **BR-022（删除策略）：** 删除用户标记为"已离职"，保留历史关联数据。
*   **BR-023（自动保存）：** 数据修改后行级即时保存。
...
##### 业务规则
*   **BR-024（项目全称唯一）：** 项目全称必须唯一，使用无业务含义的ID作为主键。
*   **BR-025（删除策略）：** 删除项目不连带删除关联的项目计划和费用申报记录。
*   **BR-026（自动保存）：** 数据修改后行级即时保存。
...
##### 业务规则
*   **BR-027（备份内容）：** 备份文件包含所有用户数据、项目、任务、费用申报、个人记事、纪念日等数据实体的JSON数据文件。
...
##### 业务规则
*   **BR-028（导入格式）：** 支持CSV/Excel格式导入。
*   **BR-029（错误处理）：** 导入失败时提示具体错误行号和错误原因，提供重试选项。
...
##### 业务规则
*   **BR-032（日期规则）：** 支持按公历/农历日期设置，以年为周期；闰年2月29日仅在闰年有效；农历闰月按农历历法处理。
*   **BR-033（自动保存）：** 数据录入时自动保存，无需手动保存按钮。
...
|---|---|---|---|---|---|
| 用户 | 系统用户账户 | - 用户ID (主键，自增ID)<br>- 用户名 (varchar(20))<br>- 密码哈希<br>- 姓名 (varchar(50))<br>- 邮箱 (varchar(100))<br>- 角色（管理员/成员/项目经理）<br>- 状态（启用/禁用） | 永久保存 | 用户名唯一，使用无业务含义的ID作为主键 |
...
### 8.3. 业务规则详述
- **BR-001至BR-033** 的完整描述和配置说明详见各章节。

## 数据迁移方案
支持CSV/Excel格式导入，具体方案：
1. 页面上传方式：管理员选择文件并上传
2. 字段映射：提供字段映射界面，让管理员确认导入数据与系统字段的对应关系
3. 数据校验：系统校验数据格式，包括必填字段、数据类型、日期格式等
4. 错误处理：导入失败时提示具体错误行号和错误原因，提供重试选项
5. 导入反馈：显示导入成功/失败统计，并提供重试选项
6. 支持的数据类型：文本、日期、数字（小数点后两位）

## 待确认事项
1. **农历日期字段处理**：当前设计为基于公历日期自动计算且不可编辑，与用户需求调研时表示的"农历日期可选"存在差异。建议确认最终的处理方式。
